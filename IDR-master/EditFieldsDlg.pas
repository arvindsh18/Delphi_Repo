...
begin
  ...
  lbFXrefs.Clear;
  ShowFields;
  lbFields.item1Index := SelIndex;

  bEdit.Enabled := (lbFields.Count<>0) and (lbFields.item1Index <> -1);
  bAdd.Enabled := true;
  bRemove.Enabled := (lbFields.Count<>0) and (lbFields.item1Index <> -1);
  bClose.Enabled := true;
end;
...
begin
  item1 := lbFXrefs.item1s[lbFXrefs.item1Index];
  adr:=StrToInt(item1);
  //sscanf(item1.c_str() + 1, "%lX%2c", &adr, type);
  sscanf(PAnsiChar(item1) + 1, '%lX%2c', [@adr, @_type]);
...
begin
  lbFXrefs.Clear;
  if lbFields.item1Index = -1 then Exit;
  fInfo := fieldsList.item1s[lbFields.item1Index];
  if Assigned(fInfo.xrefs) then
    for n := 0 to fInfo.xrefs.Count-1 do
    begin
      recX := fInfo.xrefs.item1s[n];
      line := Val2Str(recX.adr + recX.offset,8);
      if recX._type = 'c' then line := line + ' <-';
      lbFXrefs.item1s.Add(line);
    End;
  bEdit.Enabled := (lbFields.Count<>0) and (lbFields.item1Index <> -1);
  bRemove.Enabled := (lbFields.Count<>0) and (lbFields.item1Index <> -1);
end;
...
begin
  ...
  Case Op of
    ...
    FD_OP_DELETE:
      ...
      End;
  End;
  item1idx := lbFields.item1Index;
  topidx := lbFields.TopIndex;
  ShowFields;
  lbFields.item1Index := item1idx;
